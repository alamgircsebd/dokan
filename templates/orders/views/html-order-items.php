	...
	<table cellpadding="0" cellspacing="0" class="woocommerce_order_items dokan-table dokan-table-strip">
		<thead>
			<tr>
				...
				<th class="item sortable" colspan="2" data-sort="string-ins"><?php _e( 'Item', 'dokan-pro' ); ?></th>

				<?php do_action( 'woocommerce_admin_order_item_headers' ); ?>

				<th class="item_cost sortable" data-sort="float"><?php _e( 'Cost', 'dokan-pro' ); ?></th>
				<th class="quantity sortable" data-sort="int"><?php _e( 'Qty', 'dokan-pro' ); ?></th>
				<th class="line_cost sortable" data-sort="float"><?php _e( 'Total', 'dokan-pro' ); ?></th>

				<?php
					if ( empty( $legacy_order ) && ! empty( $order_taxes ) ) :
						foreach ( $order_taxes as $tax_id => $tax_item ) :
							$tax_class      = wc_get_tax_class_by_tax_id( $tax_item['rate_id'] );
							$tax_class_name = isset( $classes_options[ $tax_class ] ) ? $classes_options[ $tax_class ] : __( 'Tax', 'dokan-pro' );
							$column_label   = ! empty( $tax_item['label'] ) ? $tax_item['label'] : __( 'Tax', 'dokan-pro' );
							?>
...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
		...
		if ( $coupons ) {
			...
			<div class="wc-used-coupons">
				<ul class="wc_coupon_list"><?php
					echo '<li><strong>' . __( 'Coupon(s) Used', 'dokan-pro' ) . '</strong></li>';
	...
	<table class="wc-order-totals">
		<tr>
			<td><?php _e( 'Discount', 'dokan-pro' ); ?> <span class="tips" data-tip="<?php _e( 'This is the total discount. Discounts are defined per line item.', 'dokan-pro' ); ?>">[?]</span>:</td>
			<td class="total">
		...
		<tr>
			<td><?php _e( 'Shipping', 'dokan-pro' ); ?> <span class="tips" data-tip="<?php _e( 'This is the shipping and handling total costs for the order.', 'dokan-pro' ); ?>">[?]</span>:</td>
		...
		<tr>
			<td><?php _e( 'Order Total', 'dokan-pro' ); ?>:</td>
			<td class="total">
		...
		<tr>
			<td class="refunded-total"><?php _e( 'Refunded', 'dokan-pro' ); ?>:</td>
...
<div class="wc-order-data-row wc-order-bulk-actions">
	...
	<p class="add-items">
		...
		<?php if ( ( $order->get_total() - $order->get_total_refunded() ) > 0 ) : ?>
			<button type="button" class="dokan-btn dokan-btn-default refund-items"><?php _e( 'Request Refund', 'dokan-pro' ); ?></button>
		<?php endif; ?>
	</p>
	<div class="clear"></div>
</div>
...
<div class="wc-order-data-row wc-order-refund-items" style="display: none;">
	<table class="wc-order-totals dokan-table dokan-table-strip">
		...
		<tr>
			<td><label for="refund_reason"><?php _e( 'Reason for refund (optional)', 'dokan-pro' ); ?>:</label></td>
			<td class="total">
	...
	<div class="refund-actions">
		...
		<button type="button" class="dokan-btn dokan-btn-default do-manual-refund tips" data-tip="<?php esc_attr_e( 'You will need to manually issue a refund through your payment gateway after using this.', 'dokan-pro' ); ?>"><?php printf( _x( 'Submit Refund Request %s', 'Submit Refund Request $amount', 'dokan-pro' ), $refund_amount ); ?></button>
		<button type="button" class="dokan-btn dokan-btn-default cancel-action"><?php _e( 'Cancel', 'dokan-pro' ); ?></button>
		<div class="clear"></div>
	</div>
	<div class="clear"></div>
</div>
		...
		<div class="wc-backbone-modal-content">
			<section class="wc-backbone-modal-main" role="main">
				<header class="wc-backbone-modal-header">
					...
					<h1><?php _e( 'Add products', 'dokan-pro' ); ?></h1>
				</header>
				<article>
					<form action="" method="post">
						<input type="hidden" id="add_item_id" name="add_order_items" class="wc-product-search" style="width: 100%;" data-placeholder="<?php _e( 'Search for a product&hellip;', 'dokan-pro' ); ?>" data-multiple="true" />
					</form>
				</article>
				<footer>
					<div class="inner">
						<button id="btn-ok" class="button button-primary button-large"><?php _e( 'Add', 'dokan-pro' ); ?></button>
					</div>
				</footer>
			</section>
		</div>
	...
	<div class="wc-backbone-modal">
		<div class="wc-backbone-modal-content">
			<section class="wc-backbone-modal-main" role="main">
				<header class="wc-backbone-modal-header">
					<a class="modal-close modal-close-link" href="#"><span class="close-icon"><span class="screen-reader-text">Close media panel</span></span></a>
					<h1><?php _e( 'Add tax', 'dokan-pro' ); ?></h1>
				</header>
				<article>
					<form action="" method="post">
						<table class="widefat">
							<thead>
								<tr>
									<th>&nbsp;</th>
									<th><?php _e( 'Rate name', 'dokan-pro' ); ?></th>
									<th><?php _e( 'Tax class', 'dokan-pro' ); ?></th>
									<th><?php _e( 'Rate code', 'dokan-pro' ); ?></th>
									<th><?php _e( 'Rate %', 'dokan-pro' ); ?></th>
								</tr>
							</thead>
						<?php
						...
						<?php if ( absint( $wpdb->get_var( "SELECT COUNT(tax_rate_id) FROM {$wpdb->prefix}woocommerce_tax_rates;" ) ) > 100 ) : ?>
							<p>
								<label for="manual_tax_rate_id"><?php _e( 'Or, enter tax rate ID:', 'dokan-pro' ); ?></label><br/>
								<input type="number" name="manual_tax_rate_id" id="manual_tax_rate_id" step="1" placeholder="<?php _e( 'Optional', 'dokan-pro' ); ?>" />
							</p>
						<?php endif; ?>
					</form>
				</article>
