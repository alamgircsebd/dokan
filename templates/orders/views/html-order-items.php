...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
			...
			<div class="wc-used-coupons">
				<ul class="wc_coupon_list"><?php
					...
					foreach ( $coupons as $item_id => $item ) {
						...
						echo '<li class="code"><a href="' . esc_url( $link ) . '" class="tips" data-tip="' . esc_attr( wc_price( $item['discount_amount'], array( 'currency' => dokan_get_prop( $order, 'get_order_currency', 'get_currency' ) ) ) ) . '"><span>' . esc_html( $item['name'] ). '</span></a></li>';
					}
				?></ul>
			</div>
			<?php
	...
	<table class="wc-order-totals">
		<tr>
			...
			<td class="total">
				<?php echo wc_price( $order->get_total_discount(), array( 'currency' => dokan_get_prop( $order, 'get_order_currency', 'get_currency' ) ) ); ?>
			</td>
			<td width="1%"></td>
		</tr>
		...
		<tr>
			...
			<td class="total"><?php echo wc_price( $order->get_total_shipping(), array( 'currency' => dokan_get_prop( $order, 'get_order_currency', 'get_currency' ) ) ); ?></td>
			<td width="1%"></td>
		</tr>
		...
		<tr>
			...
			<td class="total refunded-total">-<?php echo wc_price( $order->get_total_refunded(), array( 'currency' => dokan_get_prop( $order, 'get_order_currency', 'get_currency' ) ) ); ?></td>
			<td width="1%"></td>
		</tr>
...
<div class="wc-order-data-row wc-order-refund-items" style="display: none;">
	<table class="wc-order-totals dokan-table dokan-table-strip">
		...
		<tr>
			...
			<td class="total">-<?php echo wc_price( $order->get_total_refunded(), array( 'currency' => dokan_get_prop( $order, 'get_order_currency', 'get_currency' ) ) ); ?></td>
		</tr>
		<tr>
			<td><?php _e( 'Total available to refund', 'dokan' ); ?>:</td>
			<td class="total"><?php echo wc_price( $order->get_total() - $order->get_total_refunded(), array( 'currency' => dokan_get_prop( $order, 'get_order_currency', 'get_currency' ) ) ); ?></td>
		</tr>
		<tr>
	...
	<div class="refund-actions">
		<?php
		$refund_amount            = '<span class="wc-order-refund-amount">' . wc_price( 0, array( 'currency' => dokan_get_prop( $order, 'get_order_currency', 'get_currency' ) ) ) . '</span>'; ?>
