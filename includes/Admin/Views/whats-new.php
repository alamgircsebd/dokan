...
$changelog = array(
    array(
        ...
        'changes'  => array(
            ...
            array(
                'title'       => 'Refactor Stripe Connect Module Update',
                'type'        => 'Fix',
                'description' => 'Refactor and fix stripe connect module. Update stripe SDK version and stripe connect type.',
            ),
            array(
                'title'       => 'Add Gateway Fee Admin Report Logs Fix',
                'type'        => 'Fix',
                'description' => 'Add gateway fee payee indicator in admin report logs. Now the admin will have a proper view of the gateway fee amount and who is paying that.',
            ),
            array(
                'title'       => 'Booking Confirmation from the Booking List Fix',
                'type'        => 'Fix',
                'description' => 'When the vendor tries to confirm booking from the booking list not working properly and not showing a thank you message.',
            ),
            array(
                'title'       => 'Activate Modules During Plugin Activation Fix',
                'type'        => 'Fix',
                'description' => 'The modules will now be inactive by default after plugin installation, enabling the modules you need.',
            ),
            array(
                'title'       => 'Product Grouped Type Fix',
                'type'        => 'Fix',
                'description' => 'We have fixed the issue, product type not changing to grouped when try to change.',
            ),
            array(
                'title'       => 'Branding Issue on Seller Search Fix',
                'type'        => 'Fix',
                'description' => 'Your can now have a proper search result of vendors by filtering them with brand on store listing page.',
            ),
            array(
                'title'       => 'Vendor Earning in Order Details Fix',
                'type'        => 'Fix',
                'description' => 'Now your vendors will see accurate vendor earnings in order details after the refund.',
            ),
            array(
                'title'       => 'Vendor Export Report Fix',
                'type'        => 'Fix',
                'description' => 'We have fixed the statement of your vendor\'s balance when vendors export the statement from their dashboard.',
            ),
            array(
                'title'       => 'Remove External Product Type Fix',
                'type'        => 'Fix',
                'description' => 'Remove external product type from subscription allowed product types for vendor subscription product.',
            ),
            array(
                'title'       => 'Subscription Product Price Not Saving Fix',
                'type'        => 'Fix',
                'description' => 'You can now save the subscription product price when WC auction plugin is active.',
            ),
            array(
                'title'       => 'Featured Sellers limit Fix',
                'type'        => 'Fix',
                'description' => 'On your store listing page, the featured sellers number was showing more than limit. We have fixed that.',
            ),
            array(
                'title'       => 'Product Tags add on Quick Edit Area Fix',
                'type'        => 'Fix',
                'description' => 'Product tags search experience improvement and fix the issue of not working properly on quick edit area.',
            ),
            array(
                'title'       => 'Text Domain in JS end Fix',
                'type'        => 'Fix',
                'description' => 'Text domain issue when report abuse delete in js end and translate not working properly.',
            ),
            array(
                'title'       => 'JS Console Error on Report Abuse Fix',
                'type'        => 'Fix',
                'description' => 'JS console error fixed on report abuse module from admin area edit product page',
            ),
            array(
                'title'       => 'Subscription Plan Page Design Fix',
                'type'        => 'Fix',
                'description' => 'Subscription plan page design will work properly now when different languages are used.',
            ),
            array(
                'title'       => 'Vendor Product Import Fix',
                'type'        => 'Fix',
                'description' => 'When a vendor imports a product from the dashboard then the default advanced option shows automatically, is not an expected behavior. So we fix that UI.',
            ),
            array(
                'title'       => 'Dokan Pro Email Template Fix',
                'type'        => 'Fix',
                'description' => 'Dokan Pro core email template locations update so now you can override the template file from theme.',
            ),
            array(
                'title'       => 'Store Default Geolocation Fix',
                'type'        => 'Fix',
                'description' => 'When you try to create a new product from the vendor dashboard then store default geolocation not set to the product.',
            ),
            array(
                'title'       => 'Coupon Product and Exclude Product Field Move Fix',
                'type'        => 'Fix',
                'description' => 'Coupon product and exclude product field move to search select with variations.',
            ),
            array(
                'title'       => 'Product Variation Toggle Fix',
                'type'        => 'Fix',
                'description' => 'Product variation toggle issue, variation downloadable file delete issue.',
            ),
            array(
                'title'       => 'Vendor Can Modify Other Product Fix',
                'type'        => 'Fix',
            ...
            array(
                'title'       => 'Multi Vendor Duplicate SKU Fix',
                'type'        => 'Fix',
                'description' => 'When someone try to sell this product from single product then SKU not duplicate as accepted.',
            ),
            array(
                'title'       => 'Vendor Confirmation Email Fix',
                'type'        => 'Fix',
                'description' => 'When some purchase a booking and the vendor does not get a booking confirmation email.',
            ),
            array(
                'title'       => 'Quick Update Products Fix',
                'type'        => 'Fix',
                'description' => 'Can not quick update products when product limit reached form vendor dashboard.',
            ),
            array(
                'title'       => 'CSV Import Feature Column Fix',
                'type'        => 'Fix',
                'description' => 'When vendor CSV imports from vendor dashboard and feature column make false, here checking CSV import vendor or admin.',
            ),
            array(
                'title'       => 'Export Wholesale Column Missing Fix',
                'type'        => 'Fix',
                'description' => 'The vendor will now see the export wholesale column when you export product from vendor dashboard.',
            ),
            array(
                'title'       => 'Product Add-on Type File not Show on Order Fix',
                'type'        => 'Fix',
                'description' => 'Product add-on type File upload does not show the file on vendor order.',
            ),
            array(
                'title'       => 'Auction Start End Field Disable Fix',
                'type'        => 'Fix',
                'description' => 'Auction start,end field disable from keyboard.',
            ),
            array(
                'title'       => 'Announcements Week Fix',
                'type'        => 'Fix',
                'description' => 'You will get all the announcements in time regardless of the timezone.',
            ),
            array(
                'title'       => 'Product Discount Scheduled Fix',
                'type'        => 'Fix',
            ...
            array(
                'title'       => 'Import Restriction with Subscription Fix',
                'type'        => 'Fix',
                'description' => 'When someone product with category name by using import tool that time use validation for subscription category restricted if found.',
            ),
            array(
                'title'       => 'Wholesale Customer Registration Email Fix',
                'type'        => 'Fix',
            ...
            array(
                'title'       => 'Report Select Date not Working Fix',
                'type'        => 'Fix',
            ...
            array(
                'title'       => 'Refund Issue with Decimal Number Fix',
                'type'        => 'Fix',
                'description' => 'When the vendor sent a refund request from the order details page then the total and refund amount were not compared correctly.',
            ),
        ),
    ),
    array(
