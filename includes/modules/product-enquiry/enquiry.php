...
class Dokan_Product_Enquiry {
    ...
    public function localization_setup() {
        load_plugin_textdomain( 'dpe', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
    }
    ...
    function send_email() {
        ...
        if ( !empty( $url ) ) {
            wp_send_json_error( __( 'Boo ya!', 'dpe' ) );
        }
        ...
        if ( !$seller || is_wp_error( $seller ) ) {
            $message = sprintf( '<div class="alert alert-success">%s</div>', __( 'Something went wrong!', 'dpe' ) );
        ...
        $subject = sprintf( __( '"%s" sent you a message from your "%s" store', 'dpe' ), $from_name, $this->get_from_name() );
        ...
        $success = sprintf( '<div class="alert alert-success">%s</div>', __( 'Email sent successfully!', 'dpe' ) );
    ...
    **/
    public function guest_user_settings( $settings_fields ) {
        $settings_fields['dokan_selling']['enable_guest_user_enquiry'] = array(
            ...
            'label'   => __( 'Guest Product Enquiry', 'dpe' ),
            'desc'    => __( 'Enable/Disable product enquiry for guest user', 'dpe' ),
            'type'    => 'checkbox',
            'default' => 'on'
        );
                        ...
                        <div class="row">
                            <?php if ( $guest_enquiry == 'off' ): ?>
                                <div class="col-xs-12 col-md-12 form-group">
                                    <?php _e( 'Please Login to make enquiry about this product', 'dpe' ); ?>
                                </div>
                                <div class="col-xs-12 col-md-12 form-group">
                                    <a class="btn btn-success btn-green btn-flat btn-lg " href="<?php echo add_query_arg( array( 'redirect_to' => get_permalink( $post->ID ) ), wc_get_page_permalink( 'myaccount' ) ); ?>"><?php _e( 'Login', 'dpe' ); ?></a>
                                </div>
                            <?php else: ?>
